// P2PGATE vault — PAYMENT_PROVEN box (claim opened; the courier-signed handoff record
...
//        no user receipt signature; the oracle is trusted, period); seller paid minus
//        fee. Oracle-only C' is also how an honest seller counters ANY claim: the
//        digest alone resolves it.
//   D  — claim:   HEIGHT > proofHeight + CLAIM_MATURATION_BLOCKS, user signs
//
// Registers:
...
//   R7 Long              (proofHeight << 32) | feeBps — packed ints (sigma 6 registers
//                        store tuples as Coll, so ints are packed arithmetically)
//   R8 Coll[Byte]        handoff-record id (32 B, blake2b256 of a_courier | z_courier |
...
//                        recipientAddr is the USER's USDT address (the seller pays the user)
//
// Context variables (release path only):
//   (0) Coll[Byte]  payment-proof payload (112 B, specs/oracle-integration.md §2.2 —
//                   digest of the SELLER's USDT transfer to the R9 user address)
//
...
// during proof reduction, so the paths are selected with a Boolean if instead.)
{
  ...
  val userKey = decodePoint(SELF.R6[Coll[Byte]].get)
  val collateral = SELF.tokens(0)._2
  val useTokenId = SELF.tokens(0)._1
  val packed7 = SELF.R7[Long].get
  val proofH = (packed7 / 4294967296L).toInt
  val feeBps = (packed7 % 4294967296L).toInt
  ...
  // its packed R7; the proven box has no spare register for it (R9 carries the copied
  ...
  // Path D pays the user into OUTPUTS(0); path C′ shares the transaction with the
  // oracle box, whose contract pins its own reproduction at OUTPUTS(0) (oracle.es),
  // so the release payout goes to OUTPUTS(1).
  val userPaid =
    OUTPUTS(0).propositionBytes == proveDlog(userKey).propBytes &&
  ...
  if (HEIGHT > proofH + %%CLAIM_MATURATION_BLOCKS%% && userPaid && feeOk) {
    // Path D — claim after maturation; the user discharges proveDlog(userKey).
    proveDlog(userKey)
  } else {

...
/**
 * In-app locale support: the supported set {en, hi, sw, ar, ru} with the language
...
/** The supported language set, picker order. */
 ...
 * ru*→ru, everything else → en. Input is a bare language subtag
 * (`Locale.language`, already lowercase); region/script never change the
 * outcome.
 */
fun resolveSupportedTag(language: String?): String = when (language) {
    "hi", "sw", "ar", "ru" -> language
    else -> DEFAULT_LOCALE_TAG
}

/**
 * Effective locale → default fiat currency: en→USD, hi→INR, sw→KSH, ar→USD
 * (EGP is not in the currency list, so Arabic falls back to USD), ru→RUB.

...
/**
 * P2PH handoff record (§3.2) wire format — the single-signed (seller key)
 * cash-received acknowledgment. Expected bytes are built independently in each
 * test from the raw fields — never by calling `encode()` — and decoded back;
 * the timestamp is asserted to sit at bytes 48..51.
 */
class MessagesSpec {
    ...
    @Test
    fun `handoff record encodes to the hand-built 52 byte fixture`() {
        val record = HandoffRecord(dealId, amount, currency, timestamp)
        assertContentEquals(expectedBytes("P2PH"), record.encode())
        assertEquals(52, record.encode().size)
    }
    ...
    fun `handoff timestamp sits at bytes 48 to 51`() {
        ...
        assertEquals(timestamp, timestampFrom(HandoffRecord(dealId, amount, currency, timestamp).encode()))
    }
    ...
    fun `decode rejects truncated input`() {
        assertFailsWith<IllegalArgumentException> { HandoffRecord.decode(expectedBytes("P2PH").copyOfRange(0, 51)) }
    }
    ...
    fun `construction rejects negative amounts and out of range timestamps`() {
        assertFailsWith<IllegalArgumentException> {
            HandoffRecord(dealId, -1, currency, timestamp).encode()
        }
        assertFailsWith<IllegalArgumentException> {
            HandoffRecord(dealId, amount, currency, 0x1_0000_0000L).encode()
        }
        assertFailsWith<IllegalArgumentException> {
            HandoffRecord(dealId, amount, "EG".encodeToByteArray(), timestamp).encode()
        }
    }
}

 ...
 * (HANDOFF_RECORD_MAX_AGE) and courier clock sanity (COURIER_CLOCK_SKEW),
 * then the Schnorr half. All defects return `false`.
 */
class CourierRecordVerifierSpec {
    ...
    private fun valid(): Triple<ByteArray, ByteArray, ByteArray> {
        ...
        val sig = ErgoTestFixtures.courierSign(record)
        return Triple(record.encode(), sig.a, sig.z)
    }

    @Test
    fun `a complete courier-signed record verifies against the terms`() {
        val terms = ErgoTestFixtures.dealTerms()
        val record = ErgoTestFixtures.handoffRecord(terms, tsSec = recordTs)
        val sig = ErgoTestFixtures.courierSign(record)
        assertTrue(
            CourierRecordVerifier.verify(record.encode(), sig.a, sig.z, terms.courierPubKey, terms, now),
        )
    }
    ...
    fun `record bound to a different deal fails`() {
        ...
        assertFalse(
            CourierRecordVerifier.verify(record.encode(), sig.a, sig.z, otherTerms.courierPubKey, otherTerms, now),
        )
    }
    ...
    fun `amount mismatch against the terms fails`() {
        val terms = ErgoTestFixtures.dealTerms()
        // Record honestly signed for a *different* amount under the same courier key:
        ...
        val sig = ErgoTestFixtures.courierSign(foreignRecord)
        assertFalse(
            CourierRecordVerifier.verify(foreignRecord.encode(), sig.a, sig.z, terms.courierPubKey, terms, now),
        )
    }
    ...
    @Test
    fun `absurd future timestamp beyond courier clock skew fails`() {
        val terms = ErgoTestFixtures.dealTerms()
    ...
    fun `garbage bytes fail decode`() {
        ...
        assertFalse(CourierRecordVerifier.verify(ByteArray(10), a, z, terms.courierPubKey, terms, now))
        val badMagic = record.copyOf().also { it[0] = 'X'.code.toByte() }
        assertFalse(CourierRecordVerifier.verify(badMagic, a, z, terms.courierPubKey, terms, now))
    }
    ...
    fun `broken signature fails`() {
        ...
        assertFalse(CourierRecordVerifier.verify(record, a, badZ, terms.courierPubKey, terms, now))
        val wrongKey = TestKeys.of(0x9999)
        assertFalse(CourierRecordVerifier.verify(record, a, z, wrongKey.pubKeyCompressed, terms, now))
    }
}

...
| `PAYMENT_CONFIRMED` | Oracle confirmed the seller's USDT transfer to the buyer (off-chain); release pending — no buyer action is required or awaited | attested `dealId` reference, timeout countdown | release via the oracle's attestation alone (preferred, automatic), else timeout reclaim (attested but unreleased) |
...
- Trigger: at `QUOTED`, before vault funding, on the buyer's declared USDT **receive** address (the address registered with the operator/oracle watch set at funding — the seller pays the buyer; since 2026-09-21 it is no longer pinned on-chain as an R9 `recipientAddr`). A re-check is triggered if the address submitted at funding differs from the declared one (address-swap defense).
...
1. **Contest** — present the oracle's attestation of the seller's USDT transfer (vault path C′): the box pays the seller, the claim is defeated. **Mechanical whenever the attestation exists — the attested `dealId` alone is sufficient, so an honest seller always counters a false claim.** There is no withheld-signature corner in v2: nothing the buyer can do or refuse blocks path C′.
...
GET  /v1/deals/{id}/attestation      # oracle attestation proxy → { status, dealId }
                                     # (dealId-only payload since 2026-09-21: no digest/srcTxId/
                                     #  block coordinates — those moved to the oracle's off-chain API)

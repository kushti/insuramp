...
*Implementation specification for the operator-side backend of the vault-insured cash→USDT on-ramp. Depends on: `specs/deal-protocol.md` (canonical deal state machine), `specs/vault-contract.md` (ErgoScript vault and its timings: `RECLAIM_TIMEOUT` 24h, `CLAIM_MATURATION` 12h, protocol fee 25–100 bps), `specs/oracle-integration.md` (payment-proof oracle for the USDT leg: phase-1 centralized NFT-authenticated oracle gating release, phase-2 Rosen-derived threshold), `specs/courier-app.md` (courier client). Covers the first asset leg only — cash→USDT on-ramp with USE collateral; the seller locks the vault. Product context: `onramp-ux.md` §4 (operator dashboard); economics context: `onramp-business-model.md` §4 (capital dynamics) and §7 (regulatory posture). The reverse direction (USDT→cash) is out of scope and not specified.*

**Implementation status (M3-B, 2026-09-17):** the backend has landed in `backend/`
(Kotlin/Ktor pinned 3.0.3, 90 tests green across 10 suites): deal engine as sole
transition authority, chain watcher, vault manager (fund/reclaim/release/contest),
quote publisher with capacity + pause gates, courier/user/dashboard `/v1` APIs +
...
- **Courier key delivery:** the endpoint returns the deal-scoped courier pubkey + id;
  delivery of the private half to the courier device is a courier-app concern
  (§9, courier API).

## 1. Scope & role

The operator is the **capital-heavy side** of the marketplace: it locks USE collateral into vaults, publishes quotes, dispatches couriers, collects the cash, and sends the USDT after the cash is in hand. The backend is the operator's tooling; it is not protocol infrastructure. Per `onramp-business-model.md` §7: the protocol never touches fiat and never custodies user funds; money-transmitter obligations attach to the operator's courier leg alone. The backend therefore handles fiat only as *records* (amounts, handoff records), never as money movement.
...
2. **Deal orchestration.** Drive each deal through the canonical state machine from `specs/deal-protocol.md` §1: `QUOTED → FUNDED → PAYMENT_PENDING → PAYMENT_CONFIRMED → RELEASED`, with quote expiry (`QuoteExpired`) closing a deal that never funded, the timeout branch `FUNDED → RECLAIMED` (user no-show), and the dispute branch `PAYMENT_PENDING → CLAIM_OPENED → CLAIMABLE → CLAIMED`. `PAYMENT_CONFIRMED` means the oracle confirmed the seller's USDT transfer; the release follows without any user action. On-chain, the vault box either spends directly `FUNDED box → spent` (routine release via path C or reclaim via path A) or detours `FUNDED box → PAYMENT_PROVEN box → spent` (dispute); the backend mirrors, never invents, these states.
...
│        │                                       │          │
│  ┌─────▼─────────┐  ┌──────────────┐  ┌───────▼────────┐ │
│  │ user deal API │  │ courier API  │  │ dispute inbox  │ │
│  └───────────────┘  └──────────────┘  └────────────────┘ │
└──────────────────────────────────────────────────────────┘
         │ REST/WS              │ REST            │ dashboard (operator keys)
```
...
- **Chain watcher.** Polls the Ergo explorer API (default) or a local node (optional, preferred for privacy) for the operator's vault boxes. Maps on-chain reality to the deal machine's events: vault box found → `VaultFunded` (→ `FUNDED`); box moved to `PAYMENT_PROVEN` (user opened a claim with the courier-signed handoff record) → `ClaimOpened` (→ `CLAIM_OPENED`); box spent via the release path (oracle digest alone) → `ReleaseObserved` (→ `RELEASED`); box spent via the timeout path → `ReclaimTimeoutElapsed` (→ `RECLAIMED`); box spent via the claim path → `ClaimPaid` (→ `CLAIMED`). (`PAYMENT_PENDING` is an off-chain transition driven by the courier API's handoff record — event `CashCollected`; `PAYMENT_CONFIRMED` is an off-chain transition driven by the oracle client — event `PaymentConfirmed`; the FUNDED box is untouched in both. A `PaymentConfirmed` arriving while a claim is open sets the deal's contested flag rather than being rejected.) Confirmation-depth policy per `specs/oracle-integration.md`.
- **Vault manager.** Builds and submits vault transactions: fund (create `FUNDED` box with deal parameters, including `courierPubKey` in the deal terms — pinned in the vault's R7, which packs `oracleNftId || courierPubKey`), reclaim (timeout path after `RECLAIM_TIMEOUT`), release (path C: oracle digest of the seller's USDT transfer, alone — there is no receipt signature to collect), contest (path C′: the same digest from the PAYMENT_PROVEN box during a claim). Reclaim is **automated as one job**, but it is state-aware: the scheduler reclaims vaults past `RECLAIM_TIMEOUT` **only for deals in FUNDED or PAYMENT_CONFIRMED** — never PAYMENT_PENDING (§1) — which is also the privacy-preserving default path (routine reclaims leave no on-chain link between vaults).
- **Quote publisher.** Maintains the operator's live quotes (spread, ETA promise, max deal size) and serves them to the user-facing API. Hard-gated by the infra monitor (§8): no quotes while verification is degraded.
- **Courier-facing API.** Endpoints consumed by `specs/courier-app.md`: route list, deal-scoped courier key delivery, handoff-record template and submission (the `"P2PH"` QR content), offline-queue sync.
- **User-facing deal API.** Deal status feed, quote feed, and an **oracle attestation proxy**: the user app never talks to the oracle directly; the backend proxies and caches confirmation status so the user's "USDT confirmed" indicator and the operator's are the same fact.
...
|---|---|---|---|
| `QUOTED` | User selected a quote and shared their USDT receive address; vault funding in progress or awaiting confirmation | quoted amount, quote expiry, receive-address fingerprint | cancel/expire before funding (`QuoteExpired`; no on-chain footprint) |
| `FUNDED` | Vault box on-chain, USE locked, courier dispatched, awaiting the meeting | locked collateral, timeout countdown (`RECLAIM_TIMEOUT` from funding height) | timeout reclaim — auto-job, but only while no handoff record exists (§1) |
| `PAYMENT_PENDING` | Cash collected (courier-signed handoff record on file); the seller must send the USDT | record timestamp, freshness countdown, courier/cash status | **none** — reclaim from here is a theft path and is rejected; the user's answer is the claim |
| `PAYMENT_CONFIRMED` | Oracle confirmed the seller's USDT transfer to the user (off-chain); release pending — no user action is required or awaited | oracle digest reference, timeout countdown | release via oracle digest alone (preferred, automatic), else timeout reclaim (attested but unreleased) |
| `RELEASED` | Vault spent via the release path | release tx id, protocol fee deducted (25–100 bps), cycle duration (feeds utilization stats) | terminal — collateral returns to pool |
| `RECLAIMED` | Vault spent via the timeout path (user no-show, or attested-but-unreleased fallback) | idle time regained | terminal |
| `CLAIM_OPENED` | User opened a claim with the courier-signed handoff record | evidence view (§7): handoff record vs oracle digest, contest deadline | contest (path C′), or accept (let it mature to CLAIMED) |
| `CLAIMABLE` | Claim matured (`CLAIM_MATURATION` elapsed), payout imminent | — | last-chance contest only |
| `CLAIMED` | Collateral paid out to the user (seller never paid, or contest lost) | loss recorded | terminal |
...
- average lock duration by exit path (a release on confirmed payment should be ≪ a timeout reclaim — that gap is the margin argument for courier speed and for prompt release submission once the oracle attests);
...
- **Spread** — the operator's margin over reference rate, in bps. Must internally exceed the protocol fee (25–100 bps) plus courier and ops costs; the publisher warns if spread < protocol fee + configured cost floor.
- **ETA promise** — minutes from `FUNDED` (deal accepted, vault locked) to the cash-collection meeting. This is a courier-network property, not a chain property; the publisher derives the default from recent realized meeting times.
- **Max deal size** — vault capacity (§4), period.

The **insured badge** on the user side reads the actual vault collateral ("insured up to $X" = USE locked in that deal's vault, per `onramp-ux.md` §2.1 and §4). The badge is the vault: overstating capacity is self-defeating because the badge is verifiable on-chain and a quote that outruns collateral either fails to fund (deal stuck in `QUOTED` until the quote expires — `QuoteExpired`) or ships a smaller badge than promised. The backend enforces this structurally — it refuses to publish a quote whose max size exceeds mix-ready collateral — rather than trusting operator discipline.

Quotes are versioned and expire (default TTL aligned with, and shorter than, `RECLAIM_TIMEOUT`; a quote must never outlive the vault funded from it). Quote feed deltas are pushed over WebSocket to the user app; full snapshot on connect.
...
- Trigger: at `QUOTED`, before vault funding, on the user's declared USDT **receive** address (the address pinned as R9 `recipientAddr` — the seller pays the user). A re-check is triggered if the address submitted at funding differs from the declared one (address-swap defense).
...
One row per open claim (deals in `CLAIM_OPENED` / `CLAIMABLE`). A claim asserts "cash was collected and the seller never paid"; it is gated on-chain by the courier-signed handoff record (path B — the v2 single-signature shape, landed in `contracts/` 2026-09-13). The evidence view from `onramp-ux.md` §4 shows the two sides of the story:

- **Handoff record** — the courier-signed `"P2PH"` record, its timestamp, and the GPS/override record from the meeting: proof the cash was collected. Path B verifies the courier's Schnorr half in-script against the R7-pinned `courierPubKey`, so a valid record is what put the box in PAYMENT_PROVEN; the view still renders the record and the meeting metadata for the operator.
- **Oracle payment status** — whether the oracle has confirmed the seller's USDT transfer to the user (digest reference, attestation id, confirmation depth per `specs/oracle-integration.md`). If yes, the claim is without cause and the contest path is mechanical. A `PaymentConfirmed` event arriving while a claim is open sets the deal's contested flag — it is never rejected — and this row is where the operator sees it.
...
1. **Contest** — present the oracle digest of the seller's USDT transfer (vault path C′): the box pays the seller, the claim is defeated. **Mechanical whenever the digest exists — the digest alone is sufficient, so an honest seller always counters a false claim.** There is no withheld-signature corner in v2: nothing the user can do or refuse blocks path C′.
2. **Accept** — concede the claim; take no action; the claim matures and path D pays the user. Rational when the seller genuinely did not pay, or when contesting costs more than the collateral at stake. The loss is recorded against the deal.
3. **Escalate / investigate** — courier-side fraud case: the handoff record verifies but the operator's evidence says the cash was never collected (rogue courier, collusion attempt). This is an operational-security matter — GPS records, dual-control, courier credential revocation — not a contract case; the action flags the courier credential and routes the deal to the operator's internal investigation. It changes nothing on-chain.
...
**Auto-pause rule (per `onramp-ux.md` §4, verbatim intent): never sell insurance you can't currently verify.** When the oracle leg is degraded, the quote publisher withdraws all quotes (user feed shows no quotes, not stale quotes) and no new vaults are funded. In-flight deals are unaffected — USDT confirmations and release co-signatures queue and apply when the oracle recovers, and the user's claim path never depends on oracle liveness (it is gated on the courier-signed handoff record, not the oracle). Auto-pause events are recorded with cause and duration; they feed the operator's realized-uptime stats, since every paused hour is idle capital.
...
**User-facing deal API** (consumer: user PWA; auth: deal-scoped bearer token — random token in the deal link per `onramp-ux.md` §2.4, minted at `QUOTED`, expires at deal close):
...
POST /v1/deals/{id}/claim            # open claim (guides user to on-chain claim tx with the handoff record)
```

The API exposes canonical state names verbatim — no parallel UI vocabulary to drift.

**Courier-facing API** (consumer: `specs/courier-app.md`; auth: deal-scoped courier token, minted at dispatch, expires at deal close; tokens are per-deal so a stolen phone can fake nothing — per `onramp-ux.md` §3):

```
GET  /v1/courier/jobs                        # route list: deal id, amount, neighborhood, status
GET  /v1/courier/jobs/{dealId}/key           # deal-scoped courier credential delivery (dispatch):
                                           # returns {dealId, courierId, courierPubKey}; the private
                                           # half's provisioning to the courier device is a courier-app
                                           # concern (landed M3-B, see the status note)
GET  /v1/courier/jobs/{dealId}/handoff       # handoff-record template (the "P2PH" QR content)
POST /v1/courier/jobs/{dealId}/handoff       # submit the signed record + GPS/override
POST /v1/courier/jobs/{dealId}/panic         # safety trigger (courier-app.md §6)
POST /v1/courier/jobs/{dealId}/geo-confirm   # soft GPS check w/ optional override reason code
POST /v1/courier/sync                        # offline queue upload (idempotent)
```
...
POST /v1/couriers/{id}/revoke          # courier credential revocation (rogue-courier case, §7)
...
- ~~**Withheld-signature-plus-claim corner**~~ **— ELIMINATED in v2 (2026-09-13).** In the old design a user who both withheld the USDT-receipt signature and opened a without-cause claim blocked path C′ (digest **+ signature** required) and the claim matured and paid — a known residual, deliberately not contract-prevented. The v2 release paths are gated on the oracle attestation **alone**, so path C′ needs only the digest: an honest seller mechanically counters any false claim, and there is nothing for the user to withhold. No known residual remains on the contest path. The surrounding posture is unchanged for *other* fraud classes: oracle fraud stays ex-post provable (public attestation, per-deal bounds — `specs/oracle-integration.md` §5.3), and courier-side record fraud (a verifying record for cash never collected) remains an operational case with real-world recourse/blacklisting against the courier credential and the deal identity.
...
- **Oracle attestation proxy caching.** How stale may a cached attestation be before the user app must show "verification delayed" rather than the last-known state? Couples to the auto-pause thresholds in §8.
- **Multi-courier dispatch.** *(Still open — M3-B ships single-courier dispatch.)* Current spec assumes one courier per deal, deal-scoped token and deal-scoped signing key. Real networks want reassignment (courier no-show → re-dispatch without cancelling the vault). Token and courier-key re-mint flow unspecified — note the deal terms pin `courierPubKey` at QUOTED and the vault pins it in R7 at funding, so reassignment after funding is impossible without a contract-level key-rotation story; reassignment is only cleanly possible before funding.
...
- **Fee accounting.** *(Still open.)* Protocol fee (25–100 bps) is deducted in-contract; the backend must reconcile the on-chain deduction against its own books. Whether fee parameters are read from chain config or hard-pinned per deployment is undecided.
...
- Vault contract, timings (`RECLAIM_TIMEOUT` 24h, `CLAIM_MATURATION` 12h), protocol fee (25–100 bps): `specs/vault-contract.md` (§2)
- Payment-proof oracle (USDT leg): `specs/oracle-integration.md`
- Courier client: `specs/courier-app.md`

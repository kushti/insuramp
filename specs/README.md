...
| 4 | `specs/android-app.md` | buyer-facing native Android app: modules, screens, chain interaction, handoff-record verification | draft; `:core:dealprotocol` + `:core:ergo` chain layer landed (`apps/core/`, M2/M3-A); the Android shell (`:app`) is not started |
| 5 | `specs/operator-backend.md` | Ktor operator backend: vault lane, collateral management, quotes, AML hook, dispute inbox, APIs | **landed M3-B** (2026-09-17, `backend/`, 87 tests green; documented deviations in the spec's status note) |
...
1. **Constants live in one place.** `RECLAIM_TIMEOUT` (24h), `CLAIM_MATURATION` (12h),
   `HANDOFF_RECORD_MAX_AGE`, `HANDOFF_CLOCK_SKEW`, `BTC_DEADLINE` (~6h), and
   `PROTOCOL_FEE_BPS` (25, compile-time) are defined only in
...
6. **Buyer app** — the physical loop; the Android shell is not started.
